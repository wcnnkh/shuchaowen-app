	...
	UploadImage.prototype = {
		...
		initUploader: function() {
				...
				switch(type) {
					...
					case "uploadStart":
						uploader.option("formData", {
							"OSSAccessKeyId":files.OSSAccessKeyId,
							"policy":files.policy,
					...
					case 'startUpload':
						index = 0;
						uploader.option('server', "http://simingtang-img.oss-cn-shanghai.aliyuncs.com");
			...
			$upload.on('click', function() {
				...
				if(state === 'ready') {
					webUploadOSS(uploader, editor.appendApiParams(editor.getOpt("serverUrl") + "/" + editor.getOpt("ossSignApiServletPath")));
					...
					'onsuccess': function(r) {
						try {
							...
							if(json.state == 'SUCCESS') {
								...
								var listLen = parseInt(json.list.length);
