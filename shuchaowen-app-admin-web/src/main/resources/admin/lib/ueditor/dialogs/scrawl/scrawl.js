 ...
 */
function getSignWhen(signUrl, file) {
	return $.when(
		$.getJSON(signUrl, {"suffix":file.ext}, function(data) {
			file.key = data.key;
			file.OSSAccessKeyId = data.accessId;
...
function webUploadOSS(uploader, signUrl) {
	var promiseArray = new Array();
	for(var i = 0; i < uploader.getFiles().length; i++) {
		var file = uploader.getFiles()[i];
		if(file.key){
			continue;
		}
		
		promiseArray.push(getSignWhen(signUrl, file));
	}
...
function exec(scrawlObj) {
	console.log(scrawlObj);
	if(scrawlObj.isScrawl) {
		addMaskLayer(lang.scrawlUpLoading);
		var base64 = scrawlObj.getCanvasData();
		if(!!base64) {
			$.getJSON(editor.appendApiParams(editor.getOpt("serverUrl") + "/" + editor.getOpt("ossSignApiServletPath")), {"suffix":"png"}, function(data) {
				var form = new FormData();
				form.append("key", data.key);
				form.append("OSSAccessKeyId", data.accessId);
				form.append("policy", data.policy);
				form.append("Signature", data.signature);
				form.append("file", convertBase64UrlToBlob(base64));
				$.ajax({
					url: "http://simingtang-img.oss-cn-shanghai.aliyuncs.com",
					data: form,
					processData: false,
					contentType: false,
					type: 'POST',
					success:function(){
						if(!scrawlObj.isCancelScrawl) {
								var imgObj = {},
									url = editor.options.scrawlUrlPrefix + data.key;
								imgObj.src = url;
								imgObj._src = url;
								imgObj.alt = "";
								imgObj.title = "";
